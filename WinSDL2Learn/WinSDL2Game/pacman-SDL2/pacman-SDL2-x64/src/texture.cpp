...
Texture& TextureManager::Load(const std::string& name,
                              ...
                              const SDL_Color& keycolor) {
    return textures_.emplace(name, Texture(renderer_, filename, keycolor, std::nullopt)).first->second;
}
...
Tilesheet& TextureManager::LoadTilesheet(const std::string& name,
                            ...
                            const SDL_Color& keycolor,
                            int col, int row) {
    auto& texture = textures_.emplace(name, Texture(renderer_, filename, keycolor, tilesheets_.size())).first->second;
...
Tilesheet* TextureManager::FindTilesheet(const std::string& name) {
  if (auto it = textures_.find(name); it != textures_.end()) {
      ...
      return &tilesheets_[texture.GetTilesheetIdx().value()];
    }
    return nullptr;
}
