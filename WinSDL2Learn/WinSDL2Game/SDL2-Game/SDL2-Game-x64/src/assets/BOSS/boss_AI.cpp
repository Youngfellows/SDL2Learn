...
void boss_AI::update()
{
	...
	case ROCKETS:
	{
		if (innerStage <= 0) { //Move boss to position
			...
			for (int i = 0; i < missiles.size() / 3; i++) {
				missiles.at(i)->setPos(spawnPos); spawnPos.y += 80; missiles.at(i)->reset();
			}
			spawnPos.set(-930, 200);
			for (int i = missiles.size() / 3; i < 2 * missiles.size() / 3; i++) {
				missiles.at(i)->setPos(spawnPos); spawnPos.y += 80; missiles.at(i)->reset();
			}
			spawnPos.set(-1330, 700);
			for (int i = 2 * missiles.size() / 3; i < missiles.size(); i++) {
				missiles.at(i)->setPos(spawnPos); spawnPos.y += 80; missiles.at(i)->reset();
			}
			innerStage = 2;
		}
