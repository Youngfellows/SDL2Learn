...
bool GameEngine::InitGameEngine()
{
	...
	if (TTF_Init() != 0)
	{ // check if TTF can be used and if it can't, then show a message
...
void GameEngine::PaddleHumanMoveMouse()
{ // Map paddleHuman's y position to the mouse's Y pos
	paddleHuman->spriteDestRect.y = mouseY - 72.5;
}

void GameEngine::UpdateBallPosition()
{ // Change destRect's x and y values to the x and y variables from the function's argument.
	ball->spriteDestRect.x += speed_x;
	ball->spriteDestRect.y += speed_y;
}

void GameEngine::ReverseBallPositionY()
{ // check ball's y position is above the top wall (y less than zero)
	if (ball->spriteDestRect.y < 0 || ball->spriteDestRect.y >(WINDOW_HEIGHT - ball->spriteDestRect.h))
	{ // or if y position is lower than the bottom wall (y greater than WINDOW_HEIGHT)
		speed_y = -speed_y;
	}
}

void GameEngine::ResetBallPositionX()
{ // If ball goes out on sides, left and right, reset to centre of screen
...
void GameEngine::PaddleAIMove()
{ // AI paddle moves with the ball (in the y-direction)
	paddleAI->spriteDestRect.y = ball->spriteDestRect.y - paddleAI->spriteDestRect.h / 2 + ball->spriteDestRect.h / 2;
}

void GameEngine::ResetPaddleAIBallNotAIArea()
{ // Reset paddleAI and stop it when ball is away from right side of divider
...
void GameEngine::BallInAIArea()
{ // If ball is on the right side of the divider, paddleAI moves with improved AI
...
void GameEngine::CheckBallPaddleCollision()
{ // continually check if ball and player paddle or the ball and AI paddle collide
...
{ // Add score for AI if ball collides with paddleHuman's goal area behind paddleHuman
...
{ // Add score for Player if ball collides with paddleAI's goal area behind paddleAI
...
bool GameEngine::NotAIArea(SDL_Rect BALL, SDL_Rect AIAREA)
{
	...
	if (bottomBALL <= topAIAREA)
	{ // if any of the sides from Ball are outside of AIArea
		return false;
	}
...
void GameEngine::AI()
{ // Improve AI Paddle: making it challenging for player to win
	if (ball->spriteDestRect.y < WINDOW_HEIGHT / 2 + 2)
	{ // top: aiPaddle moves upward in direction + 2
	...
	if (ball->spriteDestRect.y >= WINDOW_HEIGHT / 2 - 2)
	{ // bottom: aiPaddle moves downward in direction - 2
		paddleAI->spriteDestRect.y = ball->spriteDestRect.y * 0.7610; // aiPaddle's downward movement is multiplied/slowed by 0.660, 0.8208
	}
}

void GameEngine::AddToPlayerScore()
{ // Scoring System: Player's Score
	p1score++;
	s1 = to_string(p1score);
	RenderFont(s1.c_str(), 190, 70, true);
}

void GameEngine::AddToAIScore()
{ // Scoring System: AI's Score
	aiscore++;
	s2 = to_string(aiscore);
	RenderFont(s2.c_str(), 595, 30, true);
}

void GameEngine::KeepPlayerScore()
{ // Game-End Condition: if AI scores 5 points
	if (aiscore == 5)
	{
		AIWins();
	}
...
void GameEngine::KeepAIScore()
{ // Game-End Condition: if Player scores 5 points
	if (p1score == 5)
	{
		PlayerWins();
	}
...
void GameEngine::PlayerWins()
{ // Game-End Condition: Player wins
...
void GameEngine::AIWins()
{ // Game-End Condition: AI wins
	ball->spriteDestRect.x = -20;
	ball->spriteDestRect.y = -20;
	paddleAI->spriteDestRect.y = WINDOW_HEIGHT / 2 - paddleAI->spriteDestRect.h / 2 - 10;
	paddleHuman->spriteDestRect.y = WINDOW_HEIGHT / 2 - paddleHuman->spriteDestRect.h / 2 - 10;
...
void GameEngine::Input()
{
	while (SDL_PollEvent(&event))
	{
		...
		{ // if we click "X" button to close the window then SDL_Quit event type is triggered
			isRunning = false;
		}
