...
namespace Dungeon
{
	...
	void Rectangle::OnDrawCallback(DisplayObject *self, SDL_Surface *winSurace, Resource *resource)
	{
		Rectangle *rect = (Rectangle *)self->GetSubClass();
		if (rect)
		{
			rect->Draw(winSurace, resource);
		}
	}
	...
	*/
	void  Rectangle::Draw(SDL_Surface *winSurace, Resource *resource)
	{
		...
		//0xffffffff ARGB
		SDL_Rect rect = {
			(int)mRectangleData->dest->x,
			(int)mRectangleData->dest->y,
