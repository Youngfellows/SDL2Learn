...
#define FRAMERATE 60
	...
	Game::Game() : mWindow(nullptr), mIsRunning(true),
		mRenderer(nullptr), mCursor(nullptr), mResource(nullptr)
	{
	...
	bool Game::Initialize()
	{
		...
		mCursor = new Cursor(mResource);
		if (!mCursor->Cursor_Create(0, 0, 20, 20))
		{
			return false;
		}
	...
	void Game::Event()
	{
		...
		while (SDL_PollEvent(&event))
		{
			...
			case SDL_MOUSEMOTION:
				mCursor->Cursor_OnMouseMove(&event);
				break;
			default:
				break;
			}
	...
	void Game::Draw()
	{
		...
		//绘制指定要鼠标光标
		mCursor->Cursor_OnDraw(mRenderer);
