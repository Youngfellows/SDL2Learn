...
#define FRAMERATE 60
	...
	Game::Game() : mWindow(nullptr), mIsRunning(true), mRenderer(nullptr),
		mEyeball(nullptr), mLeftEye(nullptr), mRightEye(nullptr), mResource(nullptr)
	{
	...
	void Game::Draw()
	{
		...
		if (mLeftEye)
		{
			mLeftEye->DisplayObject_Draw(mResource, mRenderer);//绘制左眼球
		}
		if (mRightEye)
		{
			mRightEye->DisplayObject_Draw(mResource, mRenderer);//绘制右眼球
		}
	...
	{
		mEyeball = new Eyeball();
		mLeftEye = mEyeball->Eyeball_Create(100, 100);
		mRightEye = mEyeball->Eyeball_Create(160, 100);
	...
	void Game::FreeComponents()
	{
		if (mEyeball)
		{
			mEyeball->Eyeball_Destory(mLeftEye);
			mEyeball->Eyeball_Destory(mRightEye);
		}
	}
}