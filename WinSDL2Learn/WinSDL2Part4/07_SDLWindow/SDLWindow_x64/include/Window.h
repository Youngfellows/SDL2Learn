...
public:
	...
	static void Quit();
	//Draw an SDL_Texture
	static void Draw(SDL_Texture *tex, SDL_Rect &dstRect, SDL_Rect *clip, float angle, int xPivot, int yPivot, SDL_RendererFlip flip);
	//Load an image
	...
	static void HandleEvents(const std::function<void(SDL_Event &)> &cbFunction);//函数指针,回调函数
	//Clear window
	static void Clear();
	//Present renderer
	static void Present();
